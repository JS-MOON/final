								...
								<ul>
									...
									<li><span class="summaryT">현재 작업현황</span> <span
										class="value"><span class="count">0<span
												class="among">/<span>3</span></span></span>개</span></li>
						...
						<div class="pdtResumeWrap">
							<div class="pdtMark">
								<ul>
									...
									<li><span class="mark markType2" style="display: none;">총알
											전달 가능</span></li>
								</ul>
										...
										<c:forEach var="i" begin="0" end="4" step="1">
											<c:if test="${op[i*2]!=null}">
												<li><span class="input-check"> <input
														type="checkbox" name="ops" id="${op[i*2+1]}"
														value="${op[i*2] }" onchange="opplus();" />
												</span> <span class="label">${op[i*2] }</span> <span
													class="optionPrice">${op[i*2+1] }</span></li>
											</c:if>
										</c:forEach>
									</ol>
									<input type="hidden" name="completedOption"
										id="completedOption" value="" /> <input type="hidden"
										name="basicPrice" id="basicPrice" value="${dto.brPrice }" />
									<input type="hidden" name="mainPhoto"
										value="${dto.brMainPhoto}" /> <input type="hidden"
										name="category1" value="${category1 }" /> <input
										type="hidden" name="category2" value="${category2 }" /> <input
										type="hidden" name="subject" value="${dto.brSubject }" /> <input
										type="hidden" name="brNum" value="${dto.brNum}">
								</div>
							</div>
							<div class="amount">
								총 금액&nbsp; <span class="price"> <input type="text"
									name="totalPrice" id="totalPrice" value="${dto.brPrice }"
									class="price" readonly="readonly"
									style="text-align: right; border: 0px;" />
								</span> 원
							</div>
							<div class="btnArea al_r">
								<a class="btnType2" href=""><span>문의하기</span></a> <a
									class="btnType3" href="javascript:order()"><span>구매하기</span></a>
							</div>
						</div>
						<!-- 제품상세 -->
					</div>
				...
				<div class="pdt-block">
					<div class="pdtDetailWrap">
						...
						<div class="pdtDetail" style="text-align: center">
							<img class="pdtDetail" src="../Product/${dto.brMorePhoto}" /> <img
						...
						<div class="pdtReview">
							<div id="pdtReviewBlock" class="tabMenu">
								...
								<div id="photoReview"
									class="tabContents photoReview boardSection">
									...
									<form action="comments_ok.action" method="post" name="rateForm">
										<table>
											<tr>
												<td colspan="2"><textarea name="cm_content"
														class="textarea" title="소개" placeholder="평가를 남겨주세요."
														style="width: 100%; height: 64px; resize: none;"></textarea>
												</td>
											</tr>
											<tr>
												<td><select name="cm_rating" class="input-text"
												...
												<td><a class="btnType12" href='javascript:comment()'
													style="float: right; height: 27px;"><span>등록</span></a></td>
											</tr>
										</table>
										<input type="hidden" name="cm_nickName"
											value="${sessionScope.session.mbNickName}"> <input
											type="hidden" name="brNum" value="${dto.brNum}">
									</form>
								</div>
							</div>
					...
					<div class="pdtRelated">
						...
						<c:forEach var="dto" items="${relists }">
							<ul>
								<li><a href="GDetail.action?brNum=${dto.brNum }"> <span
										class="thumb"> <img
											src="../Product/${dto.brMainPhoto }"
											alt="7일간 유지되는 당신만의 타투디자인을 만들어 드립니다. " Height=160px
											Width=160px />
